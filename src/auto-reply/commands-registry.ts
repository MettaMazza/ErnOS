...
import type { ErnOSConfig } from "../config/types.js";
...
export function isCommandEnabled(cfg: ErnOSConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ErnOSConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ErnOSConfig),
...
export function getCommandDetection(_cfg?: ErnOSConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ErnOSConfig) {
