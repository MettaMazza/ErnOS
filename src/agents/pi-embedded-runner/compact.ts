...
import { resolveErnOSDocsPath } from "../docs-path.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveErnOSAgentDir();
  await ensureErnOSModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createErnOSCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveErnOSDocsPath({
      workspaceDir: effectiveWorkspace,
