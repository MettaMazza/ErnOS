...
import { createErnOSTools } from "./ernos-tools.js";
...
function resolveExecConfig(params: { cfg?: ErnOSConfig; agentId?: string }) {
...
export function createErnOSCodingTools(options?: {
  agentId?: string;
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createErnOSReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
