...
function createDefaultThreadConfig(): LoadedConfig {
  return {
    ...
    session: { store: "/tmp/ernos-sessions.json" },
...
describe("discord tool result dispatch", () => {
  it(
    ...
    async () => {
      const cfg = createMentionRequiredGuildConfig({
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bernos\\b"] },
        },
      });
      ...
      await handler(createGuildMessageEvent({ messageId: "m2", content: "ernos: hello" }), client);
      ...
      createGuildMessageEvent({
        ...
        messagePatch: {
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "ErnOS" },
          },
        },
        eventPatch: {
  ...
  it("forks thread sessions and injects starter context", async () => {
    ...
    expect(capturedCtx?.SessionKey).toBe("agent:main:discord:channel:t1:u2");
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/ernos" },
      session: { store: "/tmp/ernos-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
  ...
  it("scopes thread sessions to the routed agent", async () => {
    ...
    const capturedCtx = getCapturedCtx();
    expect(capturedCtx?.SessionKey).toBe("agent:support:discord:channel:t1:u2");
