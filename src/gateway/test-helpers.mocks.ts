...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `ernos-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.ERNOS_CONFIG_PATH = path.join(root, "ernos.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "ernos.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "ernos-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadErnOSPlugins: () => pluginRegistryState.registry,
  };
});

process.env.ERNOS_SKIP_CHANNELS = "1";
process.env.ERNOS_SKIP_CRON = "1";
process.env.ERNOS_SKIP_CHANNELS = "1";
process.env.ERNOS_SKIP_CRON = "1";
