...
type ConsoleConfigLoader = () => ErnOSConfig["logging"] | undefined;
const loadConfigFallbackDefault: ConsoleConfigLoader = () => {
  try {
    const loaded = requireConfig?.("../config/config.js") as
      | {
          loadConfig?: () => ErnOSConfig;
        }
      | undefined;
...
function normalizeConsoleLevel(level?: string): LogLevel {
  ...
  if (!level && process.env.VITEST === "true" && process.env.ERNOS_TEST_CONSOLE !== "1") {
    return "silent";
  }
...
function resolveConsoleSettings(): ConsoleSettings {
  let cfg: ErnOSConfig["logging"] | undefined =
